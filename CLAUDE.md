...
- **Feature flags**: `src/renderer/src/stores/featureFlags.ts` — localStorage-backed (`fleet:ff`). Current flag: `v2Shell` (default `false`). When `true`, `UnifiedHeader` and `Sidebar` render their V2 variants; `false` renders the preserved V1 variants. Toggle in DevTools: `localStorage.setItem('fleet:ff', JSON.stringify({v2Shell:true})); location.reload()`. Cleanup path: when V2 ships as default, delete V1 files + dispatchers + this store.
