...
- **Feature flags**: `src/renderer/src/stores/featureFlags.ts` — localStorage-backed (`fleet:ff`). Three flags: `v2Shell` (shell components), `v2Dashboard` (dashboard view), `v2Pipeline` (task pipeline) — all default `false`. Toggle in DevTools: `localStorage.setItem('fleet:ff', JSON.stringify({v2Shell:true, v2Dashboard:true, v2Pipeline:true})); location.reload()`. `v2Shell=true` → `UnifiedHeader`/`Sidebar` render V2 variants. `v2Dashboard=true` → `DashboardView` renders `DashboardViewV2` (the triage-oriented V2 dashboard) instead of `DashboardViewV1`. `v2Pipeline=true` → `SprintPipeline` renders `SprintPipelineV2` (quiet graphite surfaces, status-only color, V2 primitives) instead of `SprintPipelineV1`. Cleanup path: when each phase ships as default, delete V1 files + dispatchers + this store.
