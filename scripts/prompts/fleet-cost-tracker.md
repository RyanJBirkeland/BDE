...
- Open a PR when done: `gh api repos/RyanJBirkeland/FLEET/pulls --method POST -f title="feat: FLEET cost tracker — session spend dashboard" -f body="Cost tracking view with per-session breakdown and daily/weekly totals" -f head="$(git branch --show-current)" -f base=main --jq ".html_url"`
