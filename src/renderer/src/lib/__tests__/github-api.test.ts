...
describe('getCheckRunsList', () => {
  it('returns list of check runs', async () => {
    const checkRuns = [
      ...
      { name: 'Lint', status: 'completed', conclusion: 'failure', html_url: 'https://example.com/2' },
    ]
...
describe('getReviews', () => {
  it('returns list of reviews', async () => {
    const reviews = [
      { id: 1, user: { login: 'reviewer1' }, state: 'APPROVED', body: 'LGTM', submitted_at: '2024-01-01T00:00:00Z' },
      { id: 2, user: { login: 'reviewer2' }, state: 'CHANGES_REQUESTED', body: 'Needs work', submitted_at: '2024-01-02T00:00:00Z' },
    ]
...
describe('getReviewComments', () => {
  it('returns list of review comments', async () => {
    const comments = [
      { id: 10, user: { login: 'dev' }, body: 'Fix this', path: 'src/foo.ts', line: 5, created_at: '2024-01-01T00:00:00Z', updated_at: '2024-01-01T00:00:00Z' },
    ]
...
describe('getIssueComments', () => {
  it('returns list of issue comments', async () => {
    const comments = [
      { id: 20, user: { login: 'user1' }, body: 'Great PR!', created_at: '2024-01-01T00:00:00Z', updated_at: '2024-01-01T00:00:00Z' },
      { id: 21, user: { login: 'user2' }, body: 'Thanks!', created_at: '2024-01-02T00:00:00Z', updated_at: '2024-01-02T00:00:00Z' },
    ]
    mockGithubFetch.mockResolvedValue(ipcResponse(comments))
...
describe('createReview', () => {
  it('submits a review successfully', async () => {
    ...
    await expect(createReview('owner', 'repo', 42, {
      event: 'APPROVE',
      body: 'LGTM!',
  ...
  it('submits review with inline comments', async () => {
    ...
    const review = {
      ...
      comments: [
        { path: 'src/foo.ts', line: 10, side: 'RIGHT' as const, body: 'Fix this' },
      ],
    }
  ...
  it('throws on non-ok response with message', async () => {
    ...
    await expect(createReview('owner', 'repo', 42, { event: 'COMMENT' }))
      .rejects.toThrow('Review failed: 422 — Validation failed')
  })
  ...
  it('throws with unknown message when body has no message field', async () => {
    ...
    await expect(createReview('owner', 'repo', 42, { event: 'COMMENT' }))
      .rejects.toThrow('Review failed: 500 — unknown')
  })
})
...
describe('checkOpenPrsMergeability', () => {
  it('checks mergeability for multiple PRs in parallel', async () => {
    ...
    expect(result).toHaveLength(2)
    expect(result[0]).toEqual({ number: 1, repo: 'repo', mergeable: true, mergeable_state: 'clean' })
    expect(result[1]).toEqual({ number: 2, repo: 'repo', mergeable: false, mergeable_state: 'dirty' })
    expect(mockGithubFetch).toHaveBeenCalledTimes(2)
  })
...
describe('replyToComment', () => {
  ...
  it('throws on non-ok response with message', async () => {
    ...
    await expect(replyToComment('owner', 'repo', 42, 55, 'reply'))
      .rejects.toThrow('Reply failed: 404 — Comment not found')
  })
  ...
  it('throws with unknown when body has no message', async () => {
    ...
    await expect(replyToComment('owner', 'repo', 42, 55, 'reply'))
      .rejects.toThrow('Reply failed: 500 — unknown')
  })
})
