...
function parseNumstatSimple(numstatOut: string): Array<{ path: string; additions: number; deletions: number }> {
  return numstatOut
    .trim()
    .split('\n')
    .filter(Boolean)
...
export async function checkAutoReview(params: CheckAutoReviewParams): Promise<CheckAutoReviewResult> {
