...
FLEET's network surface is **loopback-only** — no externally reachable ports, no outbound HTTP calls beyond the Anthropic API and optional GitHub calls.
...
- **Local MCP server** (opt-in): binds `127.0.0.1:18792` when enabled via Settings → Connections → Local MCP Server. Bearer-token auth against `~/.fleet/mcp-token` (mode `0600`). Source: `src/main/mcp-server/`.
- **Outbound**:
...
## Comparison: FLEET vs Claude Code CLI

### Equivalent

| Concern                 | Claude Code CLI          | FLEET                                                          |
| ----------------------- | ------------------------ | ------------------------------------------------------------ |
...
| Token storage           | `~/.claude/` (plaintext) | `~/.fleet/oauth-token` (0600 perms), Keychain fallback removed |
| Push-to-main prevention | Permission prompts       | Prompt conventions + branch naming + review gate             |

### Where FLEET adds protection
...
### Where FLEET has more surface area

| Concern          | Claude Code CLI     | FLEET                                                                       |
| ---------------- | ------------------- | ------------------------------------------------------------------------- |
| Electron process | N/A — Node CLI only | Chromium renderer + main process                                          |
| Dependencies     | ~50                 | ~200+ (Electron, Monaco, React, etc.)                                     |
| Local data store | None                | `~/.fleet/fleet.db` — task specs, agent history (readable by local processes) |
...
**"Don't leak data over the network"** — FLEET and Claude Code CLI are effectively equivalent to external observers: FLEET's loopback listeners (`:18791` status, `:18792` MCP when enabled) do not accept connections from anything off the machine.

**Local process compromise** — If an attacker has local access as your user, both tools are equally exposed. FLEET stores more data locally (SQLite with task specs and agent history) and exposes it via the loopback MCP server when enabled — the bearer token at `~/.fleet/mcp-token` is the only gate for another local process, so treat it like any other long-lived credential (`chmod 600`, don't commit).

**Supply chain** — FLEET has a larger dependency tree due to Electron/React/Monaco. Dependencies are audited regularly via `npm audit`. As of 2026-04-03: 0 high/critical vulnerabilities, 2 moderate (Monaco's internal DOMPurify — not exploitable in FLEET's context).
...
- Database: `~/.fleet/fleet.db` (WAL mode, 0600 permissions)
- OAuth token: `~/.fleet/oauth-token` (plaintext, 0600 permissions)
- MCP bearer token: `~/.fleet/mcp-token` (plaintext, 0600 permissions; only present when MCP server is enabled)
- Logs: `~/.fleet/fleet.log` (authoritative), `~/.fleet/agent-manager.log` (legacy — may be stale from prior builds)
- Worktrees: `~/.fleet/worktrees/<repo-slug>/<task-id>/` (pipeline), `~/.fleet/worktrees-adhoc/` (adhoc agents)
