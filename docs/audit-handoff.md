...
| T-61 | P2  | ✅     | clean-code | The two `console.warn` calls in `env-utils.ts` (Keychain rotated-write failure, OAuth refresh failure) now route through `logger.warn` so the messages land in `~/.fleet/fleet.log` like every other module. (Phase 9)                                                                                                                       |
...
- **Manual `git worktree add` skips FLEET's worktree-create hook** — so `node_modules` isn't auto-symlinked. You have to do it manually.
