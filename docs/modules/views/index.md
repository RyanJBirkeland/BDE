...
| `TaskWorkbenchView.tsx` | File kept on disk but removed from `VIEW_REGISTRY` and `View` union — no longer navigable. Task creation now lives in `WorkbenchPanel` inside `PlannerView`. Still renders `GitHubOptedOutBanner` above the workbench so the read-only-mode indicator is in place if the view is ever re-registered. | `TaskWorkbenchView` (default) |
