...
- `translateCancelError(err)` — maps `TaskTransitionError` throws from `cancelTask` into `McpDomainError(McpErrorCode.InvalidTransition)` with a user-friendly message and `{ taskId, fromStatus, toStatus }` data. When `fromStatus === toStatus`, the message is `"Task is already <status>."` (catches re-cancel of already-cancelled tasks); otherwise `"Cannot transition task from '<from>' to '<to>'."`. Internal implementation details (file names, class names, bypass-prevention notes) are never propagated to MCP clients. Passes unknown errors through unchanged. Exported for unit testing.
