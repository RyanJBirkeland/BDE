...
| `localAgents.ts` | Local adhoc/assistant agent store — tracks spawned agents, log polling, and the in-flight spawn state. `spawnAgent()` forwards only `task`, `repoPath`, and `assistant` over IPC; the caller-side `model` is kept as a local display hint since the main process resolves the runtime model from `agents.backendConfig`. | `useLocalAgentsStore`, `LocalAgentProcess`, `SpawnedAgent` |
