...
3. The Connections page embeds an "About FLEET" card that duplicates the dedicated **About & Usage** tab (⌘7 → About).
...
Breakpoint behavior:
- `≤ 640px` window: padding collapses to `var(--fleet-space-4)`, content is effectively full-width.
- `640–1200px`: padding scales with `4vw`, content fills available width up to 960px.
- `≥ 1200px`: padding caps at `var(--fleet-space-8)`, content centers at 960px.
...
- **`.permissions-banner`** — accent-tinted block with `display: flex; flex-direction: column; gap: var(--fleet-space-3);` padding `var(--fleet-space-3) var(--fleet-space-4)`, `border-radius: var(--fleet-radius-md)`, `background: var(--fleet-accent-surface)`, `margin-bottom: var(--fleet-space-3)`.
- **`.permissions-banner__text`** — body text, `margin: 0`.
- **`.permissions-banner__actions`** — `display: flex; gap: var(--fleet-space-2);`.
- **`.permissions-presets`** — `display: flex; flex-wrap: wrap; gap: var(--fleet-space-2);`.
- **`.permissions-tools`** — `display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: var(--fleet-space-2) var(--fleet-space-4);`.
- **`.permissions-tool`** — `display: grid; grid-template-columns: auto auto 1fr; align-items: baseline; gap: var(--fleet-space-2); padding: var(--fleet-space-1) 0; cursor: pointer;`.
- **`.permissions-tool__name`** — `font-weight: 600;`.
- **`.permissions-tool__desc`** — `font-size: var(--fleet-size-xs); color: var(--fleet-text-muted);`.
- **`.permissions-info`** — muted explainer below the grid, `font-size: var(--fleet-size-xs); color: var(--fleet-text-muted); margin-top: var(--fleet-space-3);`.
- **`.permissions-deny-list`** — `display: flex; flex-direction: column; gap: var(--fleet-space-1); margin-bottom: var(--fleet-space-3);`.
- **`.permissions-deny-rule`** — `display: flex; align-items: center; gap: var(--fleet-space-2); padding: var(--fleet-space-1) var(--fleet-space-2); background: var(--fleet-surface); border-radius: var(--fleet-radius);`. The `<code>` child gets `flex: 1` and a monospace font already inherited.
...
Add a small `.settings-empty-state` rule to `SettingsCard.css`: `color: var(--fleet-text-muted); margin: 0;`.
...
- `ConnectionsSection.test.tsx` — drop any assertion that expects "About FLEET" text, since the card is moving out.
...
| `src/renderer/src/components/settings/__tests__/ConnectionsSection.test.tsx` | Drop About-FLEET assertions if present |
