...
- **Recommendation:** Collapse to two layers: a private `sprint-queries` (not exported from package boundary) and a public `sprint-service` that is the only entry point. The repository interface should *be* the service, not a third sibling.
