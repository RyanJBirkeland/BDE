...
- **Why it matters:** This is the exact moment where I most need clarity. If my agent did good work but it conflicts with main, I want to immediately see *which files* conflict so I can decide: rebase, manual fix in IDE, or revise. Right now I have to leave the app, `cd` to the worktree, and `git status` myself. That's the "I have to leave the app to do X" moment I most want to avoid.
...
- **Observation:** Backlog shows 40 by default, then a "Show N more" button switches to *all* tasks at once. With 200 backlog tasks (my realistic case), clicking "Show more" renders the whole list synchronously. There's no virtualization in this sidebar.
...
- **Observation:** Discard cleans up the worktree and marks task cancelled. The confirmation says "this cannot be undone." But sometimes I want to discard the *task tracking* while keeping the branch around to cherry-pick from later.
