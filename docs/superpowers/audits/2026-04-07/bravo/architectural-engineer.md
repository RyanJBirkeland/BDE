...
- **Observation:** `getCompletionsPerHour` uses `strftime('%Y-%m-%dT%H:00:00', finished_at / 1000, 'unixepoch', 'localtime')` — bucketing by *server local time* on a database that stores ISO-UTC strings. `getTaskBurndown` uses `DATE(completed_at)` against ISO strings — implicitly UTC. The two queries disagree on what "today" means.
