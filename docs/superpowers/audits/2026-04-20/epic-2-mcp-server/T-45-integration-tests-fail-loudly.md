...
`src/main/mcp-server/mcp-server.integration.test.ts:65` contains `if (!bdeConfigured) { console.warn(...); return }`. When the `bde` repo is not configured in settings (which is the default on a clean CI checkout), the test silently passes without running any assertions. `src/main/mcp-server/parity.integration.test.ts:61` has the same pattern via `it.runIf(hasBdeRepo)`. This violates F.I.R.S.T. 'Self-validating' — a green check does not prove round-trip works. An audit found the test was silently skipping on CI.
...
- `src/main/mcp-server/test-setup.ts` (new or existing) — seed a `bde` repo config in a temp DB

## Implementation

Option A (preferred): seed the repo in test setup so the precondition is always met.

Create or extend `src/main/mcp-server/test-setup.ts` with a `seedBdeRepo(db)` helper that inserts a `repos` setting via `setSettingJson`:

```ts
export function seedBdeRepo(db: Database.Database): void {
...
- A `seedBdeRepo` (or equivalent) helper exists and is called from setup.
- `it.runIf(hasBdeRepo)` and `if (!bdeConfigured) return` patterns removed.
