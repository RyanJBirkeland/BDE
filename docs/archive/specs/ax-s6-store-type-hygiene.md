...
> **Status: SHIPPED / SUPERSEDED (verified 2026-04-21). `stores/terminal.ts` and `stores/sessions.ts` (with `isAgentTab`/SubAgent shapes) no longer exist. The current store layout is sprintUI/sprintSelection/sprintFilters/etc.**
