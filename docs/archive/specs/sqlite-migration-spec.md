...
> - S3 (Task Runner): Task runner reads/writes `~/.fleet/fleet.db` directly.
...
**Date:** 2026-03-16
**Vision:** Replace Supabase REST (cloud Postgres) + `agents.json` flat file with a local SQLite database (`~/.fleet/fleet.db`). FLEET is a desktop app — local storage is faster, simpler, and more reliable.

**Why:** Zero network latency, no API keys, works offline, proper querying over flat JSON files, single source of truth for all FLEET state.
...
| ------------------------------------------ | ---------------------------- | ------------------------------------------- |
| Supabase Life OS (`sprint_tasks` table)    | Task queue, backlog, sprint  | `fleet.db` sprint_tasks                       |
| `~/.fleet/agents.json`                       | Agent run history + metadata | `fleet.db` agent_runs                         |
| `~/.fleet/agent-logs/<date>/<id>/output.log` | Agent log output             | **Stays on disk** (streaming files, not DB) |
| In-memory (hardcoded)                      | Repo paths, settings         | `fleet.db` settings                           |
...
PRAGMA journal_mode = WAL;  -- concurrent readers (FLEET + task runner)
...
CREATE TABLE IF NOT EXISTS agent_runs (
  id           TEXT PRIMARY KEY,  -- UUID, generated by FLEET/task-runner at spawn
  pid          INTEGER,
...
const DB_PATH = join(homedir(), '.fleet', 'fleet.db')
...
The task runner (`life-os/scripts/task-runner.js`) runs on the same machine. It opens the same `~/.fleet/fleet.db` file using `better-sqlite3`. SQLite WAL mode handles concurrent reads/writes safely.
...
const db = new Database(join(homedir(), '.fleet', 'fleet.db'))
...
Verification: app starts, `~/.fleet/fleet.db` exists, schema tables visible.
...
- `~/Documents/Repositories/life-os/scripts/task-runner.js` — replace all Supabase REST calls with better-sqlite3
  - `getQueuedTasks()`, `claimTask()`, `updateTask()`, `registerFleetAgent()`, `finishFleetAgent()` all use SQLite
...
**Scope:** Replace `~/.fleet/agents.json` flat file with `agent_runs` SQLite table.
...
- `src/main/agent-history.ts` — REWRITE: all read/write uses SQLite `agent_runs` table
  ...
  - Log files stay on disk at `~/.fleet/agent-logs/` — only metadata moves to SQLite
- `src/main/handlers/agent-handlers.ts` — update `agents:list` and `agents:readLog` to use new agent-history API
- `~/Documents/Repositories/life-os/scripts/task-runner.js` — update `registerFleetAgent()` / `finishFleetAgent()` to write to `agent_runs` table (coordinate with S3)
...
- [ ] FLEET starts with no Supabase env vars required
...
- [ ] npm test passes
- [ ] No Supabase calls in FLEET main process (grep check)
