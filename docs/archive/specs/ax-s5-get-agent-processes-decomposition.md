...
> **Status: SUPERSEDED (verified 2026-04-21). `local-agents.ts` and `getAgentProcesses()` no longer exist — the whole concept was rearchitected into `src/main/agent-manager/` + `src/main/adhoc-agent.ts`. Retained for historical reference only.**
