...
> **Status: SHIPPED (verified 2026-04-21).** `tokens.css` now holds the unified v2 token set; the neon theme layers on top. The dual-system era is over. Retained for historical reference. (Memory/Cost view targets no longer exist — those views were removed.)
