...
**Problem**: `pollPrStatuses` depends on `prMergedMap` and is used in a `setInterval` (line 92). Every time `prMergedMap` changes, a new `pollPrStatuses` is created, the old interval is cleared, and a new one is set. But `prMergedMap` changes *because* `pollPrStatuses` updates it — causing an infinite re-interval cycle. Each cycle also re-creates the effect at line 90-96.
